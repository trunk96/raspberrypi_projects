...
class PicklePersistenceJobs(BasePersistence):
    ...
    def __init__(
        ...
        self.chat_data_to_be_dumped: Optional[DefaultDict[int, Dict]] = None
    ...
    def load_singlefile(self) -> None:
        try:
            ...
            with open(self.filename, "rb") as f:
                ...
                self.chat_data_to_be_dumped = defaultdict(dict, data['chat_data'])
            ...
            self.chat_data_to_be_dumped = defaultdict(dict)
            self.bot_data = {}
    ...
    def dump_singlefile(self) -> None:
        with open(self.filename, "wb") as f:
            for elem in self.chat_data:
                for i in self.chat_data[elem]:
                    if i == 'job':
                        self.chat_data_to_be_dumped[elem]['job'] = None
                    else:
                        self.chat_data_to_be_dumped[elem][i] = deepcopy(self.chat_data[elem][i])
                
            data = {
                'conversations': self.conversations,
                'user_data': self.user_data,
                'chat_data': self.chat_data_to_be_dumped,
    ...
    def get_chat_data(self) -> DefaultDict[int, Dict[Any, Any]]:
        ...
        if self.chat_data:
            ...
            if not data:
                ...
                data = defaultdict(dict, data)
            self.chat_data = data
            self.chat_dat_to_be_dumped = data
        else:
            self.load_singlefile()
        return deepcopy(self.chat_data)  # type: ignore[arg-otype]
    ...
    def update_chat_data(self, chat_id: int, data: Dict) -> None:
        ...
        if self.chat_data.get(chat_id) == data:
            return
        if self.chat_data_to_be_dumped is None:
            self.chat_data_to_be_dumped = defaultdict(dict)
        self.chat_data[chat_id] = data
        if 'job' in self.chat_data[chat_id]: 
            for elem in self.chat_data[chat_id]:
                if elem != 'job':
                    self.chat_data_to_be_dumped[chat_id][elem] = deepcopy(self.chat_data[chat_id][elem])
                else:
                    self.chat_data_to_be_dumped[chat_id]['job'] = None
        else:
            self.chat_data_to_be_dumped = self.chat_data
        # HERE I HAVE DISABLED DUMP OF JOBS in CHAT_DATA 
        # in order to avoid errors in the dump (it gives an error on thread_lock when saving jobs on disk)
        if not self.on_flush:
            if not self.single_file:
                filename = "{}_chat_data".format(self.filename)
                self.dump_file(filename, self.chat_data_to_be_dumped)
            else:
                self.dump_singlefile()
                ...
                for elem in self.chat_data:
                    for i in self.chat_data[elem]:
                        if i == 'job':
                            ...
                            self.chat_data_to_be_dumped[elem][i] = deepcopy(self.chat_data[elem][i])
                self.dump_file("{}_chat_data".format(self.filename), self.chat_data_to_be_dumped)
            if self.bot_data:
